...
* No user-facing sentence may assert how a result came out unless it has a second version for when it comes out the other way. Every pre-registered test in the sequence is a valid test of a true null, so it is wrong about 5% of the time by construction - and the seed is a control the interface tells the reader to press. Scenarios 01-06 carry a `body*Hit` variant that names the false alarm and explains it; scenario 07 branches on whether the search correction rejected both finalists, because it rejects both only at the default candidate count. Test a new claim by asking which seed or slider position makes it false.
* Every scenario documents its generator twice, because "there is nothing here" is otherwise only our word for it. A collapsed "exactly how the data are made" panel in the introduction gives the three steps the code actually takes, and a closing `NullEvidencePanel` runs the pre-registered analysis on a thousand independent data sets. Show p-values when the test is continuous - a flat histogram is what a true null looks like, and scenario 06 has already taught the reader to look for it - and the scenario's own count when it is discrete, because binning a count into ten bins leaves empty bars that read as a defect. The panel replicates the pre-registered analysis only: a searched-for result has no reference distribution, which is the point of everything before it. Where the picture is not simply flat, the text says why: scenario 01's exact binomial is conservative, scenario 04 is flat only after risk adjustment, and scenario 07 holds a real relationship, so it contrasts the connected feature with an unconnected one.
* Czech prose is checked for the formulas that creep in: `a prave`, `presne tak vypada`, `Vsimnete si`, an em dash followed by `a`, rhetorical questions outside panel titles, and sentences over about 200 characters. Two short sentences beat one long one.
