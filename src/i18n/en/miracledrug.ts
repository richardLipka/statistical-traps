const miracledrug = {
  intro: {
    ...
    recipeStep1:
      'Each patient is given one shared score: how well they are doing in general.',
    recipeStep2:
      'Each outcome is then that score plus its own independent noise. That is why the outcomes move together.',
    recipeStep3:
      'Both arms are generated by the same code. Which arm a patient is in never enters the computation.',
  ...
  conclusion: {
    ...
    evidence: {
      heading:
        'Evidence that the treatment really does nothing',
      body:
        'We run {{replications}} independent trials and measure the same registered outcome in each of them.',
      run:
        'Run {{replications}} trials',
      histogramX:
        'p-value of the registered outcome',
      histogramY:
        'Number of trials',
      ariaLabel:
        'Distribution of the registered outcome’s p-values across {{replications}} independent trials.',
      caption:
        'The bars are the same height: no p-value is favoured. That is what an outcome the treatment does nothing to looks like.',
      note:
        '{{share}} of the trials came out below 0.05. Five percent is what to expect, and across {{replications}} trials that figure wobbles by about a percentage point. It is why one trial is never enough. The dependence between the outcomes themselves has not gone anywhere: it is in the data on purpose, it simply has nothing to do with the treatment.',
    },
