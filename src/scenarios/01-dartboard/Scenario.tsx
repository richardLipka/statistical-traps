...
export default function DartboardScenario() {
  ...
  }, [params.dartCount, params.seed, presetTarget])
          ...
          {stage === 'conclusion' ? (
            <section className="space-y-4">
              ...
              <NullEvidencePanel
                ...
                onRun={runEvidence}
                values={evidence?.hits ?? null}
                scale="integer"
                markerValue={evidence?.expectedHits}
                markerLabel={t('conclusion.evidence.marker')}
                formatBinLabel={(value) => formatInteger(value, locale)}
                ...
                caption={t('conclusion.evidence.caption', {
                  replications: formatInteger(EVIDENCE_REPLICATIONS, locale),
                  mean: formatNumber(evidence?.meanHits ?? 0, locale, 2),
                  expected: formatNumber(evidence?.expectedHits ?? 0, locale, 2),
                })}
